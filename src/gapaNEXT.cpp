...
std::vector<ColorProfile> EnumerateColorProfiles() {
    ...
    for (const auto& entry : fs::directory_iterator(colorDir)) {
        ...
        if (ext != L".ICM" && ext != L".ICC") continue;
...
struct IccFile {
    ...
    IccFile(const std::wstring& path) : h(INVALID_HANDLE_VALUE), tagCount(0) {
        h = CreateFileW(path.c_str(), GENERIC_READ, FILE_SHARE_READ,
        ...
        if (!ReadFile(h, header, sizeof(header), &br, nullptr) || br < 128) {
            CloseHandle(h); h = INVALID_HANDLE_VALUE; return;
        }
        ...
        if (tagCount > 200) { CloseHandle(h); h = INVALID_HANDLE_VALUE; return; }
    }

    ~IccFile() { if (h != INVALID_HANDLE_VALUE) CloseHandle(h); }
    ...
    bool GetTagEntry(int idx, char sigOut[4], DWORD& offset, DWORD& size) {
        ...
        if (!ReadFile(h, entry, 12, &br, nullptr) || br < 12) return false;
...
std::string ReadProfileDescription(const std::wstring& path) {
    ...
    for (DWORD i = 0; i < icc.tagCount; i++) {
        char sig[4];
        DWORD offset, size;
        if (!icc.GetTagEntry(i, sig, offset, size)) break;

        if (memcmp(sig, "desc", 4) != 0 || size <= 4) continue;

        std::vector<BYTE> data;
        if (!icc.ReadTagData(offset, data, size)) continue;
            ...
            if (size >= 28) {
                ...
                if (numRecords > 0 && 28 + rs <= size) {
                    ...
                    for (DWORD r = 0; r < numRecords && r < 10; r++) {
                        ...
                        if (lang == 0x0001) { recOff = 28 + r * rs; break; }
                        if (r == 0) recOff = 28;
                    }
                    ...
                    if (strLen > 0 && strOff + strLen * 2 <= size) {
                        ...
                        for (DWORD c = 0; c < strLen && wd[c]; c++) wdesc += wd[c];
...
GammaRamp ReadProfileGammaRamp(const std::wstring& path) {
    ...
    for (DWORD i = 0; i < icc.tagCount; i++) {
        ...
        DWORD offset, size;
        if (!icc.GetTagEntry(i, sig, offset, size)) break;
        if (memcmp(sig, "vcgt", 4) != 0 || size < 14) continue;

        std::vector<BYTE> data;
        if (!icc.ReadTagData(offset, data, size) || size < 14) break;
        ...
        if (gammaType == 0) {
            ...
            if (numEnt < 2 || numEnt > 65535) break;
            ...
            if (!usePlanar && isMono(readInterleaved)) usePlanar = false;
            ...
            if (numEnt == GAMMA_RAMP_SIZE) {
                ...
                for (int j = 0; j < GAMMA_RAMP_SIZE; j++) {
                    ...
                    auto lerp = [](WORD a, WORD b, double t) -> WORD {
                        return (WORD)(a + (b - a) * t);
                    };
                    if (usePlanar) {
...
bool ApplyGammaToDisplay(const GammaRamp& ramp, const std::string& deviceName) {
    ...
    // SetDeviceGammaRamp takes a non-const pointer despite not modifying it
    BOOL result = SetDeviceGammaRamp(hdc, const_cast<LPVOID>(static_cast<LPCVOID>(&ramp)));
    DeleteDC(hdc);

    if (!result) {
        std::cerr << "Error: SetDeviceGammaRamp failed (code " << GetLastError() << ")" << std::endl;
        return false;
    }
    return true;
}
...
static void CmdGamma(int profileIdx) {
    ...
    if (icc) {
        for (DWORD i = 0; i < icc.tagCount; i++) {
            ...
            DWORD offset, size;
            if (!icc.GetTagEntry(i, sig, offset, size)) break;
            if (memcmp(sig, "vcgt", 4) != 0) continue;
            ...
            if (icc.ReadTagData(offset, data, size)) {
                ...
                for (DWORD b = 0; b < (std::min<DWORD>)(30, size); b++) {
                    ...
                    printf("%02X ", data[b]);
                    if (b % 16 == 15 || b == (std::min<DWORD>)(30, size) - 1) std::cout << std::endl;
                }
                ...
                std::cout << "bytes[12..17] as subheader: numCh=" << v12
                          << " numEntries=" << v14 << " entrySize=" << v16 << std::endl;
                std::cout << "With subheader: 18+" << v12 << "*" << v14 << "*" << v16
                          << "=" << (18 + v12*v14*v16) << " vs size=" << size << std::endl;
...
static void CmdApply(int profileIdx, int displayIdx) {
    ...
    for (const auto& d : targets) {
        std::cout << "  " << d.name << " ... ";
        if (ramp.valid) {
            if (ApplyGammaToDisplay(ramp, d.deviceName))
                std::cout << "OK (gamma ramp)" << std::endl;
            else
                std::cout << "FAILED" << std::endl;
        } else {
            std::cout << "WARNING: profile has no VCGT tag, gamma not changed." << std::endl;
            std::cout << "         Try a display calibration profile with gamma table data." << std::endl;
        }
    }
}
...
static void CmdReset(int displayIdx) {
    ...
    for (const auto& d : targets) {
        std::cout << "  " << d.name << " ... ";
        if (ApplyGammaToDisplay(identity, d.deviceName))
            std::cout << "OK (reset to linear)" << std::endl;
        else
            std::cout << "FAILED" << std::endl;
    }
}
...
static void CmdStatus() {
    ...
    for (const auto& d : displays) {
        std::cout << "  " << d.name << " (" << d.deviceName << ")"
                  << " -> " << (d.currentProfile.empty() ? "(default)" : d.currentProfile)
                  << std::endl;
    }
    std::cout << std::endl;
}
...
static void CmdTest() {
    std::cout << std::endl << "Gamma ramp diagnostic:" << std::endl;
    std::cout << "======================" << std::endl;
    ...
    for (const auto& d : displays) {
        std::cout << std::endl << "Display: " << d.name << " (" << d.deviceName << ")" << std::endl;

        // Test 1: CreateDC for specific display
        HDC hdc = CreateDCA(d.deviceName.c_str(), nullptr, nullptr, nullptr);
        if (!hdc) {
            DWORD err = GetLastError();
            std::cout << "  CreateDCA(" << d.deviceName << ") FAILED, err=" << err << std::endl;

            // Try with just "DISPLAY"
            hdc = CreateDCA("DISPLAY", nullptr, nullptr, nullptr);
            if (!hdc) {
                std::cout << "  CreateDCA(DISPLAY) FAILED, err=" << GetLastError() << std::endl;
            } else {
                std::cout << "  CreateDCA(DISPLAY) OK" << std::endl;
            }
        } else {
            std::cout << "  CreateDCA OK, hdc=" << hdc << std::endl;
        }

        if (!hdc) {
            // Try GetDC(NULL) as fallback
            hdc = GetDC(NULL);
            if (hdc)
                std::cout << "  GetDC(NULL) OK, hdc=" << hdc << std::endl;
            else
                std::cout << "  GetDC(NULL) FAILED" << std::endl;
        }

        if (!hdc) continue;

        // Test 2: Check device caps
        int cmCap = GetDeviceCaps(hdc, COLORMGMTCAPS);
        std::cout << "  COLORMGMTCAPS: " << cmCap << std::endl;

        // Test 3: GetDeviceGammaRamp
        GammaRamp ramp = {};
        if (GetDeviceGammaRamp(hdc, &ramp)) {
            ramp.valid = true;
            std::cout << "  GetDeviceGammaRamp: OK (R[0]=" << ramp.red[0]
                      << " R[128]=" << ramp.red[128]
                      << " R[255]=" << ramp.red[255] << ")" << std::endl;
        } else {
            std::cout << "  GetDeviceGammaRamp: FAILED, err=" << GetLastError() << std::endl;
        }

        // Test 4: SetDeviceGammaRamp with identity
        GammaRamp identity = BuildIdentityRamp();
        if (SetDeviceGammaRamp(hdc, &identity)) {
            std::cout << "  SetDeviceGammaRamp(identity): OK" << std::endl;
        } else {
            std::cout << "  SetDeviceGammaRamp(identity): FAILED, err=" << GetLastError() << std::endl;
        }

        // Test 5: Try a slightly modified ramp
        GammaRamp slight = identity;
        slight.red[128] += 100;
        if (SetDeviceGammaRamp(hdc, &slight)) {
            std::cout << "  SetDeviceGammaRamp(slight): OK - restoring..." << std::endl;
            SetDeviceGammaRamp(hdc, &identity);
        } else {
            std::cout << "  SetDeviceGammaRamp(slight): FAILED, err=" << GetLastError() << std::endl;
        }

        // Test 6: Try the VCGT ramp from profile 3 directly via this hdc
        GammaRamp vcgt = ReadProfileGammaRamp(L"C:\\Windows\\System32\\spool\\drivers\\color\\tarkov_sb_n.icc");
        if (vcgt.valid) {
            std::cout << "  VCGT R[0]=" << vcgt.red[0] << " R[255]=" << vcgt.red[255] << std::endl;

            // Try writing via raw WORD array (sans bool member)
            WORD rawRamp[3][256];
            for (int j = 0; j < 256; j++) {
                rawRamp[0][j] = vcgt.red[j];
                rawRamp[1][j] = vcgt.green[j];
                rawRamp[2][j] = vcgt.blue[j];
            }
            if (SetDeviceGammaRamp(hdc, rawRamp)) {
                std::cout << "  SetDeviceGammaRamp(VCGT via WORD[3][256]): OK - restoring..." << std::endl;
                SetDeviceGammaRamp(hdc, &identity);
            } else {
                DWORD err = GetLastError();
                std::cout << "  SetDeviceGammaRamp(VCGT via WORD[3][256]): FAILED, err=" << err << std::endl;
            }

            // Try without the valid field padding (packed struct)
            #pragma pack(push, 1)
            struct PackedRamp { WORD r[256]; WORD g[256]; WORD b[256]; };
            #pragma pack(pop)
            PackedRamp packed;
            memcpy(packed.r, vcgt.red, 512);
            memcpy(packed.g, vcgt.green, 512);
            memcpy(packed.b, vcgt.blue, 512);
            if (SetDeviceGammaRamp(hdc, &packed)) {
                std::cout << "  SetDeviceGammaRamp(VCGT packed): OK - restoring..." << std::endl;
                SetDeviceGammaRamp(hdc, &identity);
            } else {
                std::cout << "  SetDeviceGammaRamp(VCGT packed): FAILED, err=" << GetLastError() << std::endl;
            }

            // Binary search for the blending threshold
            std::cout << "  Blend threshold search:" << std::endl;
            ...
            for (int iter = 0; iter < 20; iter++) {
                ...
                GammaRamp blend;
                for (int j = 0; j < 256; j++) {
                    blend.red[j]   = (WORD)(identity.red[j] * (1.0 - mid) + vcgt.red[j] * mid);
                    blend.green[j] = (WORD)(identity.green[j] * (1.0 - mid) + vcgt.green[j] * mid);
                    blend.blue[j]  = (WORD)(identity.blue[j] * (1.0 - mid) + vcgt.blue[j] * mid);
                }
                blend.valid = true;
                if (SetDeviceGammaRamp(hdc, &blend)) {
                    lo = mid;
                    SetDeviceGammaRamp(hdc, &identity);
                } else {
                    hi = mid;
                }
                if (hi - lo < 0.001) { threshold = lo; break; }
            }
            std::cout << "    Max VCGT blend that works: " << (int)(threshold * 100) << "%" << std::endl;

            // Check monotonicity of VCGT
            bool mono = true;
            for (int j = 1; j < 256; j++) {
                if (vcgt.red[j] < vcgt.red[j-1]) { std::cout << "    R not mono at " << j << std::endl; mono = false; }
                if (vcgt.green[j] < vcgt.green[j-1]) { std::cout << "    G not mono at " << j << std::endl; mono = false; }
                if (vcgt.blue[j] < vcgt.blue[j-1]) { std::cout << "    B not mono at " << j << std::endl; mono = false; }
            }
            if (mono) std::cout << "    All channels monotonic" << std::endl;

            // Check if any channel has non-zero at index 0 (black point)
            std::cout << "    Black point: R=" << vcgt.red[0] << " G=" << vcgt.green[0] << " B=" << vcgt.blue[0] << std::endl;
            std::cout << "    White point: R=" << vcgt.red[255] << " G=" << vcgt.green[255] << " B=" << vcgt.blue[255] << std::endl;

            // Compare struct sizes
            std::cout << "  sizeof(GammaRamp)=" << sizeof(GammaRamp)
                      << " sizeof(WORD[3][256])=" << sizeof(rawRamp) << std::endl;
        } else {
            std::cout << "  VCGT ramp not found" << std::endl;
        }
...
static void CmdHelp() {
    ...
    std::cout << "  status, s            Show system-associated profile per display" << std::endl;
    std::cout << "  apply, a <P> <D>     Apply profile P gamma to display D (0=all)" << std::endl;
    ...
    std::cout << "  gamma, g <P>         Show parsed gamma ramp values from profile P" << std::endl;
    std::cout << "  test                 Diagnose gamma ramp capability" << std::endl;
    std::cout << "  help, h, ?           Show this help" << std::endl;
    std::cout << "  exit, quit, q        Exit" << std::endl;
    std::cout << std::endl;
    std::cout << "Profiles marked [vcgt] contain gamma table data." << std::endl;
    std::cout << "Only [vcgt] profiles can change screen colors via GPU gamma ramp." << std::endl;
    std::cout << std::endl;
}
...
static void CmdTags(const std::wstring& path) {
    ...
    for (DWORD i = 0; i < icc.tagCount && i < 50; i++) {
        ...
        std::cout << "  " << std::string(sig, 4)
                  << "  offset=" << offset << " size=" << size
                  << "  type=" << (typeSig[0] ? typeSig : "?")
                  << (isGamma ? "  << GAMMA TABLE" : "") << std::endl;
    }
    std::cout << std::endl;
}
...
int main(int argc, char* argv[]) {
    ...
    if (cmd == "list") {
        ...
        if (g_cachedProfiles.empty()) std::cout << "  (none)" << std::endl;
        else PrintProfiles(g_cachedProfiles);
        std::cout << std::endl << "Displays:" << std::endl;
        auto displays = EnumerateDisplays();
        if (displays.empty()) std::cout << "  (none)" << std::endl;
        else PrintDisplays(displays);
        return 0;
    }
